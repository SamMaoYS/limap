...
class MultiScanDepthReader(_base.BaseDepthReader):
    def __init__(self, filename):
        super(MultiScanDepthReader, self).__init__(filename)
...
def read_scene_multiscan(cfg, dataset, scene_id, load_depth=False):
    # set scene id
    ...
    if load_depth:
        ...
        for img_id, imname in enumerate(imname_list):
            ...
            depth = MultiScanDepthReader(depth_fname)
            depths[img_id] = depth
